...
> **현재 버전:** `v1.4.0`  
> **라이브 서비스 URL:** [https://www.daejanggan.org/](https://www.daejanggan.org/)  
...
| 구분 | 정보 | 비고 |
| :--- | :--- | :--- |
| **라이브 도메인** | `https://www.daejanggan.org/` (기본: `115.68.168.246`)<br/>`http://ndaejanggan.iwinv.net/` (개발/서브) | 실시간 운영 중 (v1.4.0, SSL 적용) |
...
| **대표 관리자 계정** | ID: `gimac` / PW: `a14241425` | 배용하 대표님 (전체 사이트 운영 관리) |
| **개발자 계정** | ID: `admin` / PW: `#seungho0409` | 이승호 개발자님 (장애 알림 수신) |
| **텔레그램 알림 봇** | `@nuriohkr_bot` (누리오 봇) | 관리자 Chat ID: `5618137472` |
| **로컬 AI 서버** | `http://49.170.204.109:1234` | LM Studio (`google/gemma-4-e2b`) |
...
### 🛠️ 8) 도서 등록 시스템 고도화 & 안전 방어 로직 완비
- **ISBN 컬럼 `VARCHAR(100)` 확장**: 긴 세트 ISBN 및 부가 설명 포함 시에도 DB 잘림 오류 원천 차단.
- **도서 제목/부제목 양끝 기호(`< >`, `《 》`, `“ ”`, `『 』` 등) 자동 정제 필터**: 원고 첫 줄을 그대로 복사해 넣어도 깨끗한 도서명으로 자동 가공.
- **도서 등록일시(`created_at`) 컬럼 추가**: 관리자 도서 목록에서 등록 일시(`YYYY.MM.DD HH:mm`) 실시간 확인 가능.
- **도서 등록 시 `book_categories` 다중 카테고리 자동 동기화**: 등록 즉시 카테고리별 도서 목록 및 뱃지 연동.

### 📄 9) 스마트 7-윈도우 페이지네이션 전역 적용
- `1 … 28 29 [30] 31 32 … 66` 스마트 생략 스타일을 사이트 전역(회원 66P, 도서 32P, 주문, 카테고리, 게시판, 저자별)에 일괄 적용.

### 🖼️ 10) 플로팅 배너 순수 이미지 단독 노출 (`130 x 280px`)
...
### 3단계: 실서버 라이브 확인
- 브라우저에서 [https://www.daejanggan.org/](https://www.daejanggan.org/) 에 접속하여 `F5`(새로고침)로 확인합니다. (개발/임시: `http://ndaejanggan.iwinv.net/`)
